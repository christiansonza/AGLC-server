...
export const destroyPaymentRequest = async (req, res) => {
  try {
...
export const destroyPaymentRequestDetails = async (req, res) => {
  try {
    const paymentRequestId = parseInt(req.params.id, 10);

    const deletedCount = await PaymentRequestDetail.destroy({
      where: { paymentRequestId }
    });

    return res.status(200).json({
      message: `Deleted ${deletedCount} detail(s).`,
      paymentRequestId,
    });
  } catch (error) {
